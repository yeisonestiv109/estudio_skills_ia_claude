...
class TestTheirStackAdapter:
    ...
    def test_aging_alto_genera_tier0_efecto_con_fecha_evento_ahora(
        ...
        """
        Vacante abierta con date_posted de hace 80 días → aging >= 75 →
        TIER_0/EFECTO (recalibración bandas 26-jul: 45-75d=TIER_1, >=75d=TIER_0),
        con fecha_evento ≈ now (estado continuo) para que el decay de EFECTO
        (45d) de ScoreTriggerPolicy no la elimine.
        """
        from src.core.domain.models import TierUrgencia, TipoTrigger
        from src.adapters.triggers.theirstack_adapter import TheirStackAdapter

        fecha_80d = (datetime.now(timezone.utc) - timedelta(days=80)).strftime(
            "%Y-%m-%d"
        )
        vacantes = [
            self._vacante("Senior Python Dev", fecha=fecha_80d),
            self._vacante("AWS Architect", fecha=fecha_80d),
            self._vacante("Django Engineer", fecha=fecha_80d),
        ]
        respuesta_mock = self._mock_response(vacantes)
    ...
    def test_aging_usa_vacante_mas_antigua_devuelta(self, empresa: Empresa):
        ...
        Con una vacante de hace 80d y otras recientes, el aging es 80 → TIER_0
        (banda >=75d).
        """
        ...
        vacantes = [
            ...
            self._vacante("Antigua", fecha=fecha_80d),
        ]
        respuesta_mock = self._mock_response(vacantes)
    ...
    def test_payload_scoring_hibrido_pide_vacante_mas_antigua_1_credito(
        self, empresa: Empresa
    ):
        """
        VACANTE VIEJA-Y-ABIERTA (fix 26-jul-2026): el payload de scoring pide UNA
        vacante (limit=1 = 1 crédito, TheirStack cobra por REGISTRO) posteada hace
        45-90 días que SIGUE abierta (posted_at_gte/lte + is_closed=False). NO usa
        order_by (deprecado en la API → se ignora; era el bug de la corrida #4).
        """
        from src.adapters.triggers.theirstack_adapter import TheirStackAdapter
        ...
        assert "order_by" not in payload  # deprecado, no se usa
        assert payload["is_closed"] is False  # solo vacantes abiertas
        assert "posted_at_gte" in payload  # ventana: hace 90 días
        assert "posted_at_lte" in payload  # ventana: hace 45 días (>= aging TIER_0)
        assert payload["company_domain_or"] == [empresa.dominio]
    ...
    def test_payload_aging_filtra_por_rol_tecnico_a_nivel_vacante(
        self, empresa: Empresa
    ):
        """
        Fix corrida #5: la query de aging filtra por ROL TÉCNICO a nivel de
        VACANTE (job_title_pattern_or), no por company_technology_slug_or (nivel
        empresa). Así la vacante envejecida es realmente de desarrollo/eng, no un
        rol administrativo en una empresa que alguna vez publicó algo tech. El
        filtro de tecnología del ICP vive ahora SOLO en el discovery.
        """
        from src.adapters.triggers.theirstack_adapter import TheirStackAdapter
        ...
        payload = mock_post.call_args.kwargs["json"]
        # Rol técnico a nivel vacante (lista de patrones regex, no vacía).
        assert payload.get("job_title_pattern_or")
        # La query de aging NO usa el filtro de tecnología a nivel empresa.
        assert "company_technology_slug_or" not in payload
...
class TestGoogleAlertsRSSAdapter:
    ...
    def test_key_pool_compartido_se_reutiliza(self, empresa: Empresa):
        ...
        entradas = [
            _EntradaRSSMock(
                title=titular,
                summary="La empresa de software cierra ronda de inversión.",
            )
        ]
        with _rss(entradas):
            with patch(
