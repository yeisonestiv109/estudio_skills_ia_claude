...
Cubren los contratos de seguridad definidos en:
    02_Lineas_de_Producto/Outbound_Prospector/docs/modelos_dominio_core.md (v3.0)
...
# Diseño: 02_Lineas_de_Producto/Outbound_Prospector/docs/tecnico/prospector-m4-design.md
# ===========================================================================
...
class TestPoliticaExclusionCompetidores:
    ...
    def test_ambiguedad_es_simetrica_sin_importar_el_orden(self):
        ...
        assert (
            resultado_ab
            == resultado_ba
            == ResultadoExclusionCompetidor.REQUIERE_ANALISIS_SEMANTICO
        )
...
class TestPoliticaTipoOrganizacion:
    ...
    def test_todos_los_tipos_no_empresa_son_descartados(self, tipo: TipoOrganizacion):
...
# ---------------------------------------------------------------------------
# Bloque 21: PoliticaFitComprador — gate de fit de comprador (B, 26-jul-2026)
# ---------------------------------------------------------------------------
class TestPoliticaFitComprador:
    policy = PoliticaFitComprador()

    def test_multinacional_confirmada_no_es_apta(self):
        """es_multinacional=True (filial/multinacional afirmada) → NO apta."""
        assert self.policy.es_apta(True) is False

    def test_empresa_independiente_es_apta(self):
        """es_multinacional=False (PYME independiente) → apta."""
        assert self.policy.es_apta(False) is True

    def test_indeterminado_es_apta_fail_open(self):
        """None (sin señal) → apta (fail-open, como PoliticaTipoOrganizacion)."""
        assert self.policy.es_apta(None) is True

    def test_multinacional_con_secop_local_es_apta(self):
        """EXCEPCIÓN SECOP (caso Atrys): multinacional PERO con contrato público
        local activo → apta (la plata pública local valida la compra local)."""
        assert self.policy.es_apta(True, tiene_trigger_secop=True) is True

    def test_multinacional_sin_secop_no_es_apta(self):
        """Multinacional sin trigger SECOP → NO apta (fuera del ICP)."""
        assert self.policy.es_apta(True, tiene_trigger_secop=False) is False
