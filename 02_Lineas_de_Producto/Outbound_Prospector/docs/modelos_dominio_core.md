...
class OrigenTrigger(str, Enum):
    ...
    THEIRSTACK         = "THEIRSTACK"         # hiring (vacantes) vía jobs/search
...
class ManifiestoICP(BaseModel):
    ...
    industrias_objetivo: list[str]      = Field(default_factory=list, description="Industrias COMPRADORAS objetivo (las que tienen el dolor y comprarían al cliente), derivadas del ICP por el LLM. NUNCA la categoría del propio cliente (buscar por eso devuelve competidores — defecto del run #2). Vacío = discovery amplio por tech+tamaño+país. Alimenta ApolloDiscoveryAdapter (q_organization_keyword_tags). La exclusión de competidores la hace el Negative ICP semántico, no este campo.")
