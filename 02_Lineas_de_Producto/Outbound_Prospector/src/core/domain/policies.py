...
Transcripción fiel de:
    - `02_Lineas_de_Producto/Outbound_Prospector/docs/modelos_dominio_core.md` (TriggerAggregationPolicy)
    - `02_Lineas_de_Producto/Outbound_Prospector/docs/flujos_motor_1_y_2.md`   (AdapterRoutingPolicy)
...
# Diseño: 02_Lineas_de_Producto/Outbound_Prospector/docs/tecnico/prospector-m4-design.md §4, §6, §7
# ===========================================================================
...
# ICP" (02_Lineas_de_Producto/Outbound_Prospector/docs/tecnico/prospector-m3-m4-design.md, sesión de
...
class PoliticaCorroboracionTamano:
    ...
    def corroborar(
        ...
        for est in estimaciones:
            ...
            confianza_por_tier.setdefault(est.tamano_estimado, []).append(est.confianza)
    ...
    def excede_icp(
        ...
        estado: EstadoConsensoTamano,
        tamano_consensuado: TamanoEmpresa | None,
        icp_tamano: TamanoEmpresa,
    ) -> tuple[bool, TamanoEmpresa | None]:
        """
        Gate de tamaño ASIMÉTRICO (Signal-First Discovery, 25-jul-2026): decide
        si una empresa candidata es DEMASIADO GRANDE para el ICP y debe
        descartarse. Pura y determinista.

        Antes el sandbox solo descartaba CONSENSO=ENTERPRISE y solo con 2
        orígenes corroborando (fail-open ante dato único/ausente) → dejaba pasar
        scale-ups disfrazados de SME por el default de `_inferir_tamano`. Esta
        regla lo cierra con una asimetría deliberada:

          (a) Un número firmográfico DURO (employee_count de TheirStack) que
              mapee a un tier MAYOR que el del ICP basta POR SÍ SOLO para
              EXCLUIR: es un dato duro, no una inferencia semántica, así que no
              exige corroboración para DESCARTAR un "demasiado grande".
          (b) Un CONSENSO corroborado (>= MINIMO_ORIGENES) de un tier mayor al
              ICP también excluye (incluye MID_MARKET, no solo ENTERPRISE).

        Confirmar un tamaño como VÁLIDO (inclusión) sigue exigiendo 2 orígenes
        (ver corroborar); solo la EXCLUSIÓN por "demasiado grande" se permite
        con un dato duro único. Esta asimetría es el punto: preferimos NO
        malgastar esfuerzo en una empresa probablemente muy grande, aun sin
        corroboración, que arrastrar un scale-up al pipeline.

        Nota de acoplamiento: se keyea sobre OrigenTrigger.THEIRSTACK porque su
        estimación proviene de `employee_count` (entero firmográfico reportado),
        el dato más duro disponible; PropuestaValor (inferencia LLM) y SECOP
        (proxy es_pyme) son más blandos y solo pesan vía CONSENSO. Si en el
        futuro se agrega otra fuente de headcount dura, se amplía este criterio.

        Retorna (excede, tamano_reportado): tamano_reportado es el tier que
        gatilló la exclusión (para el banner), o el consensuado si no excede.
        """
        icp_ord = self._ORDEN_TIER[icp_tamano]

        for est in estimaciones:
            if (
                est.origen == OrigenTrigger.THEIRSTACK
                and self._ORDEN_TIER[est.tamano_estimado] > icp_ord
            ):
                return True, est.tamano_estimado

        if (
            estado == EstadoConsensoTamano.CONSENSO
            and tamano_consensuado is not None
            and self._ORDEN_TIER[tamano_consensuado] > icp_ord
        ):
            return True, tamano_consensuado

        return False, tamano_consensuado
    ...
    CATEGORIAS_AMBIGUAS: frozenset[frozenset[CategoriaEmpresa]] = frozenset(
        {
            ...
            frozenset({CategoriaEmpresa.AGENCIA_IT, CategoriaEmpresa.AI_ML_PLATFORM}),
        }
    )
    ...
    TIPOS_NO_APTOS: frozenset[TipoOrganizacion] = frozenset(
        {
            ...
            TipoOrganizacion.RECLUTAMIENTO_STAFFING,
        }
    )
...
class PoliticaFitComprador:
    """
    Gate de FIT DE COMPRADOR (Motor 2, 26-jul-2026). El ICP de TBBC son PYMEs
    colombianas INDEPENDIENTES 50-200 con dolor de talento tech. La corrida #5
    dejó calificar a filiales de multinacionales (Novartis Colombia, Publicis
    Media, ContourGlobal/Termopaipa): firmográficamente "SME" en su entidad
    local, pero fuera del ICP porque su compra de TI está centralizada/global,
    no la decide una consultora local. Este gate las descarta usando la señal
    semántica `es_multinacional` que el LLM ya infiere de la homepage (MISMA
    llamada cacheada de PropuestaValorAdapter — costo cero adicional).

    FAIL-OPEN (mismo espíritu que PoliticaTipoOrganizacion): un valor
    INDETERMINADO (None) se considera APTO, NO se excluye. Excluir por
    "no sé si es multinacional" mataría PYMEs locales reales cuyo homepage no
    fue claro. El gate solo MUERDE cuando el LLM afirmó POSITIVAMENTE que la
    empresa es multinacional o filial de una (es_multinacional=True).

    EXCEPCIÓN SECOP (26-jul-2026, decisión del fundador): si el LLM dice que es
    multinacional PERO la empresa tiene un trigger ACTIVO de SECOP_SOCRATA
    (contrato público local adjudicado), se PERMITE. Fundamento comercial: la
    plata pública local valida que hay una operación/compra que se decide en
    Colombia — justo el caso de Atrys Colombia (filial de multinacional española
    con contratos SECOP locales), que era el lead de mayor señal y no queremos
    perder por un ICP demasiado rígido.

    Pura y determinista: recibe dos bool | None y responde un bool. No conoce
    adaptadores ni hace red (el llamador resuelve si hay trigger SECOP).
    """

    def es_apta(
        self,
        es_multinacional: bool | None,
        tiene_trigger_secop: bool = False,
    ) -> bool:
        """
        True si la empresa es apta por fit:
          - independiente o indeterminada (es_multinacional is not True), O
          - multinacional PERO con contrato SECOP local activo
            (tiene_trigger_secop=True) → la plata pública local valida la compra.
        False SOLO si es multinacional afirmada Y sin trigger SECOP.
        No lanza excepción.
        """
        if es_multinacional is True:
            return tiene_trigger_secop
        return True
