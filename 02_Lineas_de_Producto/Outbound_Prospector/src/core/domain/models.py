...
Transcripción fiel de `02_Lineas_de_Producto/Outbound_Prospector/docs/modelos_dominio_core.md` (v3.0).
...
class TipoOrganizacion(str, Enum):
    ...
    RECLUTAMIENTO_STAFFING = "RECLUTAMIENTO_STAFFING"  # Agencias de empleo, headhunters
...
class OrigenTrigger(str, Enum):
    ...
    THEIRSTACK = "THEIRSTACK"  # Señal de vacantes (hiring) vía TheirStack jobs
...
class ManifiestoICP(BaseModel):
    ...
    industrias_objetivo: list[str] = Field(
        default_factory=list,
        description=(
            "Industrias/sectores OBJETIVO a prospectar: los que COMPRARÍAN los "
            "servicios del cliente porque tienen el dolor que el cliente resuelve. "
            "Derivadas por el LLM (Motor 1) del texto del ICP — NUNCA la categoría "
            "ni el vertical del propio cliente (buscar por eso devuelve "
            "competidores; ese fue el defecto del run #2). Ej. para una consultora "
            "IT: ['retail', 'fintech', 'logística', 'salud']. Vacío = sin filtro "
            "de industria (descubrimiento más amplio por tecnología+tamaño+país). "
            "La exclusión de competidores NO se hace aquí, es trabajo del Negative "
            "ICP semántico (Motor 2)."
        ),
    )
...
class EstimacionTamano(BaseModel):
    ...
    02_Lineas_de_Producto/Outbound_Prospector/docs/tecnico/prospector-m3-m4-design.md y hallazgos de
