...
_CLARIFICATION_QUESTIONS: dict[str, str] = {
    ...
    "categoria_empresa": (
        "¿Qué tipo de empresa es el cliente objetivo? "
        "(Ej: Agencia IT, SaaS B2B, Fintech regulada, Consultora IT)"
    ),
    "tamano_empresa": (
...
{{
  ...
  "categoria_empresa": "<VALOR_EXACTO del enum>",
  "vertical": "<sector de negocio en texto libre, Ej: Retail, Salud, Logística>",
  "industrias_objetivo": ["<industria_compradora_1>", "<industria_compradora_2>"],
  ...
  "tamano_empresa": "<VALOR_EXACTO del enum>",
  "geografia": "<código ISO Alpha-2 de 2 letras, Ej: CO, US, MX, ES>",
...
2. anclaje_tecnologico debe tener al menos 1 elemento (tecnología concreta que usa la empresa).
...
6. GEOGRAFÍA — REGLA ESTRICTA E INVIOLABLE: el campo "geografia" DEBE ser
   ...
   "ES" (España), "AR" (Argentina), "BR" (Brasil).
...
8. tamano_empresa — MAPEO NUMÉRICO ESTRICTO Y DETERMINISTA (aplica la tabla, NO interpretes):
   - menos de 50 empleados → STARTUP
   - 50 a 200 empleados (incluye "51 a 200", "100", "150 empleados") → SME
   - 201 a 1000 empleados → MID_MARKET
   - más de 1000 empleados → ENTERPRISE
   Si el texto da un RANGO (ej. "51 a 200"), clasifica por el LÍMITE SUPERIOR.
   EJEMPLO CRÍTICO: "empresas de 51 a 200 empleados" → SME (NUNCA MID_MARKET).
   Si no se menciona tamaño, usa SME por defecto.
9. industrias_objetivo — INDUSTRIAS QUE COMPRAN, NUNCA EL PROPIO CLIENTE:
   Lista de 5 a 8 sectores/industrias OBJETIVO cuyas empresas COMPRARÍAN los
   servicios del cliente porque tienen el dolor que el cliente resuelve. Es la
   demanda, no la oferta.
   PROHIBIDO incluir la categoría o el vertical del propio cliente: si el cliente
   es una consultora/fábrica de software/agencia IT, NO pongas "software",
   "consultoría IT", "desarrollo de software" ni "tecnología" (eso haría buscar
   COMPETIDORES). Pon sus CLIENTES potenciales.
   Ejemplo — cliente = consultora IT que hace backend/nube a medida:
   industrias_objetivo = ["retail", "comercio electrónico", "fintech", "logística",
   "salud", "manufactura", "educación", "inmobiliario"].
   Usa términos de industria en español (el idioma del ICP). Si el ICP nombra
   industrias objetivo explícitas, úsalas tal cual; si no las nombra, infiérelas
   del dolor operativo y del tipo de servicio del cliente. Si es imposible
   inferirlas, devuelve una lista vacía [].
...
class GroqICPAdapter(PuertoAnalizadorICP):
    ...
    def _generar_preguntas_clarificacion(exc: ValidationError) -> list[str]:
        ...
        preguntas legibles por el usuario (máximo 3, conforme al contrato del Puerto).
        """
        ...
        for campo in campos_fallidos:
            ...
            if len(preguntas) == 3:  # Límite del contrato del Puerto
                break
