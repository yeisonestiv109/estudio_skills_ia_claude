...
Diseño: 02_Lineas_de_Producto/Outbound_Prospector/docs/tecnico/prospector-m4-design.md §5, §6.
...
class ResendEnvioAdapter(PuertoEnvioCorreo):
    ...
    def __init__(
        ...
        self._remitente = remitente or os.getenv(
            "RESEND_REMITENTE", "prospector@example.com"
        )
    ...
    def enviar(self, mensaje: Mensaje, decisor: Decisor) -> ResultadoEnvio:
        ...
        try:
            ...
            logger.error(
                "Resend: error de red enviando a '%s': %s", decisor.correo, exc
            )
            return ResultadoEnvio.ERROR
...
def procesar_webhook_rebote(payload: dict) -> ResultadoEnvio | None:
    ...
    logger.debug(
        "Resend webhook: tipo de evento '%s' no mapeado. Ignorado.", tipo_evento
    )
    return None
