...
def _construir_links(
    empresa: Empresa, triggers: list[Trigger] | None
) -> LinksVerificacion:
    """
...
class PaqueteRevisionAdapter:
    ...
    def registrar_pendiente(
        ...
        if (
            ...
            and existente["estado_revision"] != EstadoRevisionHumana.PENDIENTE.value
        ):
            logger.info(
    ...
    def marcar_decision(
        ...
        datos[clave]["fecha_ultima_actualizacion"] = datetime.now(
            timezone.utc
        ).isoformat()
