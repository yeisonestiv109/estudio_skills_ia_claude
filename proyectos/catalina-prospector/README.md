# Proyecto — Catalina Rúa · El Prospector (Vía B)

Primera **oportunidad B2B activa** para el producto propio (el Prospector). Catalina Rúa es una **clienta potencial** interesada en usar el Prospector para su propio emprendimiento.

> 📌 **Aclaración de contexto:** el proyecto SEO de **TBBC lo maneja el fundador aparte**, fuera de este repo. Aquí Catalina figura **solo** como oportunidad para el Prospector, para su otra empresa/emprendimiento. No mezclar ambos frentes.

## Estado

| Campo | Valor |
|-------|-------|
| Etapa del embudo | Interés inicial (pre-diagnóstico) → **Módulo 3–4** del Vendedor Híbrido |
| Producto | El Prospector (prospección B2B autónoma con IA) |
| Módulos a afinar | **4 (entrevista/diagnóstico/cierre)** y **5 (relación a largo plazo)** |
| Bloqueante #1 | Costo por lead calificado sin medir → pricing no definido |
| Bloqueante #2 | Encaje del ICP: ¿a qué se dedica el emprendimiento de Catalina? (dato pendiente) |

## Índice

1. [`00-contexto-cliente.md`](00-contexto-cliente.md) — qué sabemos y qué NO de Catalina (sin inventar).
2. [`01-playbook-m4-entrevista.md`](01-playbook-m4-entrevista.md) — Módulo 4: diagnóstico, demo por valor, objeciones, cierre.
3. [`02-playbook-m5-relacion.md`](02-playbook-m5-relacion.md) — Módulo 5: onboarding, postventa, retención y referidos.

## Regla de este proyecto

Nada de datos inventados sobre Catalina ni su empresa. Lo que no sepamos se marca como **pendiente de confirmar** y se pregunta en la próxima interacción. El pricing no se cotiza hasta medir el costo por lead real del Prospector.
