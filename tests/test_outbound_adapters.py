...
class TestTavilyContextoAdapter:
    def test_busca_y_retorna_contexto_con_evidencias(self, empresa: Empresa, trigger: Trigger):
        payload = {
            "results": [
                {"content": "Acme SaaS anuncia expansión regional.", "url": "https://a.com/1"},
    ...
    def test_cero_resultados_retorna_contexto_vacio(self, empresa: Empresa, trigger: Trigger):
    ...
    def test_http_error_no_propaga_retorna_contexto_vacio(self, empresa: Empresa, trigger: Trigger):
    ...
    def test_timeout_no_propaga_retorna_contexto_vacio(self, empresa: Empresa, trigger: Trigger):
    ...
    def test_sin_triggers_no_lanza_y_construye_query_solo_con_empresa(self, empresa: Empresa):
        with patch("requests.post", return_value=_mock_response({"results": []})) as mock_post:
            adapter = TavilyContextoAdapter(api_key="test-key")
    ...
    def test_resultado_sin_url_o_contenido_es_omitido(self, empresa: Empresa, trigger: Trigger):
        payload = {
            "results": [
...
class TestGroqRedactorAdapter:
    ...
    def test_redacta_mensaje_valido_en_borrador(self, decisor: Decisor, empresa: Empresa, trigger: Trigger):
    ...
    def test_respuesta_sin_separador_produce_error_redaccion(self, decisor: Decisor, empresa: Empresa, trigger: Trigger):
        from src.core.domain.models import ContextoRAG
    ...
    def test_contenido_vacio_produce_error_redaccion(self, decisor: Decisor, empresa: Empresa, trigger: Trigger):
        from src.core.domain.models import ContextoRAG
    ...
    def test_rate_limit_error_no_propaga_produce_error_redaccion(self, decisor: Decisor, empresa: Empresa, trigger: Trigger):
    ...
    def test_error_inesperado_no_propaga_produce_error_redaccion(self, decisor: Decisor, empresa: Empresa, trigger: Trigger):
        from src.core.domain.models import ContextoRAG
    ...
    def test_sin_api_key_produce_error_redaccion_sin_llamar_red(self, decisor: Decisor, empresa: Empresa, trigger: Trigger):
        from src.core.domain.models import ContextoRAG
    ...
    def test_fuentes_citadas_se_propagan_desde_contexto(self, decisor: Decisor, empresa: Empresa, trigger: Trigger):
        from src.core.domain.models import ContextoRAG
...
class TestResendEnvioAdapter:
    def test_envio_exitoso_retorna_entregado(self, decisor: Decisor):
        ...
        mensaje = Mensaje(
            decisor_id=decisor.id, asunto="Asunto", cuerpo="Cuerpo", estado=EstadoMensaje.APROBADO
        )
        with patch("requests.post", return_value=_mock_response({"id": "abc123"})):
            adapter = ResendEnvioAdapter(api_key="test-key", remitente="ventas@prospector.com")
