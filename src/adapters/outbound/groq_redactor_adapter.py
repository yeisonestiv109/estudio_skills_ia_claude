...
from src.core.domain.models import ContextoRAG, Decisor, Empresa, EstadoMensaje, Mensaje, Trigger
...
class GroqRedactorAdapter(PuertoRedactorOutbound):
    ...
    def redactar(
        ...
        try:
            ...
            logger.warning("Groq redactor: rate limit para '%s': %s", empresa.nombre, exc)
            return self._mensaje_error(decisor)
        except groq_sdk.APIError as exc:
            logger.error("Groq redactor: error de API para '%s': %s", empresa.nombre, exc)
            return self._mensaje_error(decisor)
    ...
    def _construir_user_prompt(
        ...
        evidencia_texto = (
            "; ".join(contexto.evidencias[:2]) if contexto.evidencias else "sin evidencia adicional"
        )
        return (
