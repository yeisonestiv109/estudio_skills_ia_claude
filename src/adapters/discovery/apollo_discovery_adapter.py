...
FIX (defecto de filtrado — corrida real: 44 empresas, ninguna tech):
    ...
    Humanitario...). Ahora el payload también deriva DEL ICP:
      ...
      - Filtro de INDUSTRIA/KEYWORDS (q_organization_keyword_tags) desde
        manifesto.vertical y manifesto.categoria_empresa.
...
class ApolloDiscoveryAdapter(PuertoDescubridorEmpresas):
    ...
    def _construir_payload(self, manifesto: ManifiestoICP) -> dict:
        ...
        Cada filtro se DERIVA del ICP (nada hardcodeado de negocio):
          ...
          - industria/keywords ← manifesto.vertical + manifesto.categoria_empresa
    ...
    def _derivar_keyword_tags(manifesto: ManifiestoICP) -> list[str]:
        """
        Deriva los keyword tags de industria desde manifesto.vertical y
        manifesto.categoria_empresa. Transformación puramente mecánica (sin
        tabla de negocio):
          - vertical: texto libre en minúsculas ("E-commerce" → "e-commerce").
          - categoria_empresa: valor del enum con guiones bajos → espacios
            ("SAAS_B2B_HORIZONTAL" → "saas b2b horizontal"). Es el ancla de
            "sector tecnología" derivada del ICP.
        Deduplica preservando el orden.
        """
        tags: list[str] = []

        vertical = (manifesto.vertical or "").strip().lower()
        if vertical:
            tags.append(vertical)

        categoria_kw = manifesto.categoria_empresa.value.lower().replace("_", " ").strip()
        if categoria_kw and categoria_kw not in tags:
            tags.append(categoria_kw)
    ...
    def _llamar_api(self, payload: dict) -> dict | None:
        ...
        if (
            ...
            payload_degradado = {
                clave: valor for clave, valor in payload.items() if clave != _TECH_FILTER_KEY
            }
    ...
    def _es_error_filtro_avanzado(response: requests.Response) -> bool:
        ...
        try:
            ...
            return False
        return _TECH_FILTER_KEY in cuerpo or "advanced filter" in cuerpo or "upgrade" in cuerpo
