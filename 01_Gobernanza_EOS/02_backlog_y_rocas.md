...
**Nota de seguridad aparte (no bloqueante):** la key del service account de Google (`GOOGLE_SERVICE_ACCOUNT_KEY_BASE64`) quedó impresa en texto plano en la salida de una terminal durante una exploración de código esta semana -- `.env.local` está bien gitignoreado (nunca se commiteó), pero vale la pena rotarla en algún momento por higiene.

## 🧠 Sesión 30-ago-2026 (continuación) — Pausa de Groq post-agenda + Métricas del Admin (Global/Daily Metrics/Show-ups)

**Worker (`estudio_skills_ia_claude`):** el fundador ya redesplegó el Worker a Cloudflare. Decisión explícita: pausar la clasificación de agenda con Groq (`procesarSiAgendado`) -- prioridad es dejar el sistema funcionando end-to-end para lanzarse a probarlo, la automatización se retoma en unas semanas. La captura total en `activity_log` SIGUE aprobada y activa. Cambio: se comentó la llamada en `handleRequest()` de `worker_bridge_supabase_NUEVO_paralelo.js` (commit `d526f0d`), la función queda completa para reactivar después. **El Flow de ManyChat (Acciones #5 + Smart Delay) sigue sin aplicarse** -- ManyChat estuvo caído (sin acceso) esta sesión, no relacionado con nuestros cambios.

**Métricas del Admin -- spec, plan e implementación completos, en `artf-pipeline-app`:**
- Spec: `docs/superpowers/specs/2026-08-30-metricas-admin-design.md`. Plan: `docs/superpowers/plans/2026-08-30-metricas-admin.md` (todas las tareas marcadas hechas).
- Alcance acordado: Global + Daily Metrics + Show-ups ahora; Capacidad (datos manuales, fórmulas `#REF!` rotas en el Excel) queda fuera por decisión explícita del fundador.
- `vw_embudo_diario` extendida con 26 columnas nuevas (commit `ad669f2`) -- incluye `day_qbookings`/`quality_bookings_show_ups`, cuya definición NO estaba documentada en ningún lado: se dedujo por ingeniería inversa contra los 5 meses reales del Excel (`%Show Up Rate` cuadra exacto), y `estudiantes_activos`/`desistieron`/`dinero_desistido`/`cuotas_proy_ar` resueltos vía el enum real `estado_cuota` (`incobrable`=desistió). Única columna que sigue siendo inferencia razonable, no certeza: `reserva_activa` (depósitos sin venta asociada todavía -- `depositos_reserva` no tiene columna de estado propia).
- `vw_show_ups`/`fn_show_ups` nueva (commit `22383cf`), mismo patrón admin-gated que `fn_embudo_diario`. Filtro exacto: `fecha_realizada is not null` (el propio título del Excel lo define así).
- Frontend: `/metricas` pasó de vista única a 3 pestañas (`MetricasTabs.tsx` + `GlobalBoard`/`DailyMetricsBoard`/`ShowUpsBoard`, commit `ddab98a`) -- **ojo:** un primer intento de commit (`1b96c8f`) solo capturó un rename con contenido viejo por un `git add -A` con pathspec inválido que falló silenciosamente; corregido en el commit siguiente, verificar siempre `git diff --stat HEAD` después de un commit grande antes de darlo por bueno.
- Verificado con `npx playwright test` (test nuevo `e2e/metricas-admin-3-pestanas.spec.ts`, commit `c7f69be`) -- pasa standalone y junto con `auth-and-panels.spec.ts`. Correr la suite COMPLETA de 8 specs seguidas sí produjo 5 fallas (incluida esta), todas en el mismo síntoma (timeout de login/toast) -- reproducido de nuevo aparte y pasó limpio: es saturación del server de dev bajo carga secuencial larga, no una regresión real. **Anotado para la próxima vez: no tomar una falla de la suite completa como señal de bug sin aislar el test primero.**
- Todo pusheado a `origin/master` de `artf-pipeline-app`.

**Pendiente real:**
1. Aplicar el cambio de Flow de ManyChat (Acciones #5 + Smart Delay) cuando ManyChat vuelva a estar accesible.
2. Confirmar con Gaby si ya resolvió el bug de `fn_calificar_lead_con_datos` en su rama (descartado que fuera colisión, pero sigue siendo su área).
