 ...
 *
 * --- PARCHE BACKFILL (fechas históricas) ---
 * insertNewLead/updateExistingLead ahora honran fechas reales si el payload
 * trae `fecha_contacto` (opt-in real) y/o `fecha_atendido` (primer/último
 * mensaje real del bot). El tráfico EN VIVO del Worker NO manda esos campos,
 * así que sigue usando `now` exactamente como antes — comportamiento intacto.
 */
...
function insertNewLead(crm, data, now, igHandle, estado) {
  ...
  row[COL.SETTER - 1] = 'Andrew';   // nombre del bot (antes 'Javit')
  ...
  // --- PARCHE BACKFILL: fechas reales ---
  // Fecha Contacto = primer contacto real (opt-in). El backfill manda
  // fecha_contacto histórica; el tráfico en vivo no la manda → now.
  row[COL.FECHA_CONTACTO - 1] = parseDateOr(data.fecha_contacto, now);
  // Fecha Atendido = primer/último mensaje real del bot. El backfill manda
  // fecha_atendido (puede ser vacía → el bot nunca atendió → celda en blanco).
  // El tráfico en vivo no manda el campo → now (el bot atiende en este momento).
  if ('fecha_atendido' in data) {
    row[COL.FECHA_ATENDIDO - 1] = parseDateOr(data.fecha_atendido, '');
  } else {
...
function updateExistingLead(crm, rowIdx, data, now, estado) {
  ...
  // --- PARCHE BACKFILL: fechas reales ---
  // Fecha Atendido: en vivo = now; en backfill = fecha real (o blanco si nunca atendió).
  if ('fecha_atendido' in data) {
    const faReal = parseDateOr(data.fecha_atendido, '');
    if (faReal === '') {
      crm.getRange(rowIdx, COL.FECHA_ATENDIDO).setValue('');
    } else {
      setDateCell(crm, rowIdx, COL.FECHA_ATENDIDO, faReal);
    }
  } else {
    setDateCell(crm, rowIdx, COL.FECHA_ATENDIDO, now);
  }
  // Fecha Contacto: solo el backfill la corrige. En vivo NO se toca, para
  // preservar el first-seen original del lead.
  if ('fecha_contacto' in data) {
    const fcReal = parseDateOr(data.fecha_contacto, null);
    if (fcReal) setDateCell(crm, rowIdx, COL.FECHA_CONTACTO, fcReal);
  }
...
/**
 * Parsea una fecha que puede venir como ISO string del backfill
 * (ej. "2026-05-27T13:44:35-05:00") o como Date. Devuelve un Date válido,
 * o `fallback` si el valor está vacío / no es parseable.
 * Se usa para honrar fechas históricas en el backfill sin afectar el
 * tráfico en vivo (que no manda estos campos → cae al fallback `now`).
 */
...
function mapEstado(data) {
  // === PATH DIRECTO (sync-estados-via-api) ===
  // El sync ya calcula el NOMBRE EXACTO del dropdown y lo manda en `estado_directo`.
  // Si viene y es válido, lo escribimos tal cual sin pasar por el mapeo código→string.
  // Solo se acepta si está en ESTADOS_VALIDOS (si no, devuelve null y NO se escribe nada).
  // El tráfico del bot en vivo y el backfill NO mandan este campo → comportamiento intacto.
  if (data.estado_directo) {
    return ESTADOS_VALIDOS.indexOf(data.estado_directo) !== -1 ? data.estado_directo : null;
  }
  ...
  // Mapeo por etapa del bot → dropdown del CRM (versión descriptiva)
    ...
    const map = {
      ...
      // Cuando el bot (Andrew, antes "Javit") está apagado (JAVIT_ACTIVO=false), el Worker
      // registra el lead en CRM con etapa=JavitOff. Lo mapeamos a "Lead Nuevo - Sin Atender"
      // para que aparezca en la cola del equipo para procesamiento manual.
      // NOTA: el código de etapa "JavitOff" y la flag JAVIT_ACTIVO son identificadores del
      // protocolo que envía el Worker — NO se renombran aunque el bot ahora se llame Andrew.
